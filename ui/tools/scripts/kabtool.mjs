...
const archiveDir = resolveTargetPath(kabOptions.outputPath);
const inputFile = path.resolve(archiveDir, kabOptions.zipName);
const outputFile = path.resolve(archiveDir, kabOptions.kabName);
...
// Signing keyset: KOSBUILD_KEYSET (CI) or KEYS_FILE (local, cwd-relative).
// Without either, kabtool falls back to the Studio user directory's org
// keyset and fails loudly when none exists.
const keysetBuild = process.env.KOSBUILD_KEYSET;
const keysFile =
  keysetBuild ??
  (process.env.KEYS_FILE
    ? path.resolve(process.cwd(), process.env.KEYS_FILE)
    : null);
const keysetArgs = keysFile ? ['-k', keysFile] : [];
...
ps.on('close', (code) => {
  ...
  // Propagate kabtool's exit code — a failed KAB build must fail the target
  process.exit(code ?? 1);
});
