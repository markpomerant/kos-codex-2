...
/**
 * Get client response type for server API
 */
...
/**
 * Get execution context type for server API
 */
...
/**
 * Typed decorator factory for @kosServiceRequest with server API types
 *
 * Provides full IntelliSense and type safety for path, query params, and body
 * based on the server OpenAPI schema.
 *
 * @example
 * ```typescript
 * import { kosServiceRequest } from '../../utils/services/server/v1/service';
 ...
 *   // Fully typed based on server API
 * }
 * ```
 */
...
/**
 * One endpoint of the server API: a path and the method it is called with.
 */
...
/**
 * Create an API client for server
 */
