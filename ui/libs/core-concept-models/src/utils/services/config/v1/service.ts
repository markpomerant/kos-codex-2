...
/**
 * Get client response type for config API
 */
...
/**
 * Get execution context type for config API
 */
...
/**
 * Typed decorator factory for @kosServiceRequest with config API types
 *
 * Provides full IntelliSense and type safety for path, query params, and body
 * based on the config OpenAPI schema.
 *
 * @example
 * ```typescript
 * import { kosServiceRequest } from '../../utils/services/config/v1/service';
 ...
 *   // Fully typed based on config API
 * }
 * ```
 */
...
/**
 * One endpoint of the config API: a path and the method it is called with.
 */
...
/**
 * Create an API client for config
 */
